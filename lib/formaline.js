...
var fs = require( 'fs' ),
    ...
    dirSeparator = /windows/i.test(require("os").type()) ? "\\" : "/";
    emptyFn = function () {},
    dummyFn = function () { 
            ...
            try {
                if ( dataPayload ) {
                    ...
                    if ( cfg && cfg.path ) {
                        ...
                        me.fileStream.mtime = new Date();
                        var startWatch = function () {
                            if ( fs.watch ) {
                                fs.watch( cfg.path, function ( action, filename ) {
                                    fs.stat( me.uploadRootDir + filename, function ( err, stats ) {
                                        console.log("fs.watch > stat: '" + filename + "'");
                                        if ( err ) {
                                            console.log(" > error: " + err.message);
                                        }
                                        else if ( me.fileStream ) {
                                            if ( me.fileStream.path.indexOf( filename ) != -1 ) {
                                                me.fileStream.mtime = stats.mtime;
                                                console.log(" > success: mtime updated.");
                                            }
                                            else {
                                                console.log(" > error: not the fileStream file.");
                                            }

                                        }
                                        else {
                                            console.log(" > error: fileStream undefined.");
                                        }
                                        if ( stats ) {
                                            console.log(" > mtime: " + stats.mtime);
                                        }
                                        else {
                                            console.log(" > mtime: undefined");
                                        }
                                        console.log("");
                                    } );
                                } );
                                ...
                                fs.watchFile( cfg.path, function ( curr, prev ) {
                                    ...
                                    if ( me.fileStream ) {
                                        me.fileStream.mtime = curr;
                                        console.log(" > success: " + me.fileStream.path);
                                    }
                                    else {
                                        console.log(" > error: fileStream undefined");
                                    }
